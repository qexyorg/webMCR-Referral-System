...
define('MOD_STYLE', STYLE_URL.'Default/modules/qexy/refer/');				// Module style folder
define('MOD_URL', BASE_URL.'?mode=refer');									// Base module URL
...
define('MOD_CLASS_PATH', MCR_ROOT.'instruments/modules/qexy/refer/');		// Root module class folder
define('MCR_URL_ROOT', 'http://'.$_SERVER['SERVER_NAME']);					// Full base url webmcr

// Loading config
require_once(MCR_ROOT.'configs/ref.cfg.php');
...
switch($do){
	...
	case 'install':
		...
		require_once(MCR_ROOT."install_refer/install.class.php");
		$module			= new module($api);
		$mod_content	= $module->_list();
		$mod_title		= $module->title;
		$mod_bc			= $module->bc;
